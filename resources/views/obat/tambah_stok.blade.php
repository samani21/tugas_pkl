...
<form action="{{route('stok.store',$obat->kode)}}" method="POST">
    @csrf
        ...
        <div>
            ...
            <input class="form-control" type="text" id="nama_obat" name="nama_obat" value="{{$obat->nm_obat}}" placeholder="Masukkan nama" aria-label="default input example" readonly>
        </div>
        <div>
            <label for="">jumlah</label>
            <input class="form-control" type="text" id="jumlah"  name="jumlah" maxlength="10"  aria-label="default input example" autofocus>
        </div>
        <hr>
        <div>
